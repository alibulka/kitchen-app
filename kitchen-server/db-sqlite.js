...
// Добавляем done_at / skip_reason / skip_at к shift_item_status если нет
{
  ...
  if (cols.length > 0 && !cols.some(c => c.name === 'skip_reason')) {
    sqlite.exec(`ALTER TABLE shift_item_status ADD COLUMN skip_reason TEXT`);
    console.log('Migration: added skip_reason to shift_item_status');
  }
  if (cols.length > 0 && !cols.some(c => c.name === 'skip_at')) {
    sqlite.exec(`ALTER TABLE shift_item_status ADD COLUMN skip_at TEXT`);
    console.log('Migration: added skip_at to shift_item_status');
  }
}
